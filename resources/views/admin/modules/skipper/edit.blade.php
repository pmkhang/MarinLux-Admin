...
@section('content')
    <form class="w-full flex flex-col gap-8" action="{{ route('admin.skipper.update', ['id' => $skippers->id]) }}"
                ...
                <div class="intro-x w-full flex items-center gap-4">
                    <div class="w-full relative flex flex-col">
                        ...
                        <input id="phone" name="phone" type="text" class="form-control"
                            placeholder="Ex. 09xxxxxx99" value="{{ old('phone', $skippers->phone) }}">
                        @if ($errors->has('phone'))
                            <span class="text-red-500 text-sm absolute top-[68px] ">
                                {{ $errors->first('phone') }}
                            </span>
                ...
                <div class="intro-x w-full flex items-center gap-4">
                    <div class="w-full relative flex flex-col">
                        ...
                        <input id="email" name="email" type="text" class="form-control"
                            value="{{ old('phone', $skippers->email) }}">
                        @if ($errors->has('email'))
                            <span class="text-red-500 text-sm absolute top-[68px] ">
                                {{ $errors->first('email') }}
                            </span>
                    ...
                    <div class="w-full relative flex flex-col">
                        ...
                        <input id="supplier" name="supplier" type="text" class="form-control"
                            value="{{ old('supplier', $skippers->supplier) }}">
                    </div>
                </div>
